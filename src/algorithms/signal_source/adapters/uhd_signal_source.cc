    ...
    if (RF_channels_ == 1)
        {
            ...
            freq_.push_back(configuration->property(role + ".freq_hz", 1575.42e6));
            ...
            for (int i = 0; i < RF_channels_; i++)
                {
                    ...
                    freq_.push_back(configuration->property(role + ".freq_hz" + boost::lexical_cast<std::string>(i), 1575.42e6));
...
void UhdSignalSource::set_rf_freq(double new_rf_freq_hz)
{
    for (int i = 0; i < RF_channels_; i++) {
        uhd_source_->set_center_freq(new_rf_freq_hz, i);
    }
}
void UhdSignalSource::set_gain(int new_gain_db){
    for (int i = 0; i < RF_channels_; i++) {
        uhd_source_->set_gain(new_gain_db, i);
    }
}
