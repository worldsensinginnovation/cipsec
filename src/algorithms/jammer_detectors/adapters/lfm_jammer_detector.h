/*!
 * \file lfm_jammer_detector.h
 * \brief Adapts a Linear FM Jammer Detector block to a
...
#ifndef LFM_JAMMER_DETECTOR_H_
#define LFM_JAMMER_DETECTOR_H_
...
/*!
 * \brief This class adapts a Linear FM Jammer Detector block to
 *  a JammerDetectorInterface
 */

class LFMJammerDetector: public JammerDetectorInterface
{
public:
    LFMJammerDetector(ConfigurationInterface* configuration,
            std::string role, unsigned int in_streams,
            unsigned int out_streams);

    virtual ~LFMJammerDetector();
    ...
    /*!
     * \brief Returns "LFM_Jammer_Detector"
     */
    std::string implementation()
    {
        return "LFM_Jammer_Detector";
    }
    size_t item_size()
    {
        return item_size_;
    ...
    {
        detector_cc_->set_noise_estimation_done(done);
    }

    void set_noise_power_estimation(float noise_power)
    {
        detector_cc_->set_noise_power_estimation(noise_power);
    }

    float get_noise_power_estimation()
    {
        return detector_cc_->get_noise_power_estimation();
    }    

private:
    ConfigurationInterface* configuration_;
    lfm_jammer_detector_cc_sptr detector_cc_;
    gr::blocks::stream_to_vector::sptr stream_to_vector_;
    size_t item_size_;
    std::string item_type_;
    unsigned int vector_length_;
    unsigned int fft_samples_;
    unsigned int noise_estimations_;
