/*!
 * \file lfm_jammer_detector.cc
 * \brief Adapts a LFM Jammer Detector block to a JammerDetectorInterface
...
LFMJammerDetector::LFMJammerDetector(
    ...
    role_(role), in_streams_(in_streams), out_streams_(out_streams)
{
    ...
    std::string default_dump_filename = "./data/lfm_jammer_detector.dat";
    ...
    detector_cc_ = lfm_jammer_make_detector_cc(fs_in_, vector_length_, fft_samples_,
...
void LFMJammerDetector::set_threshold(float threshold)
{

    threshold_ = threshold;

    DLOG(INFO) << "LFM Jammer detector Threshold = " << threshold_;

    detector_cc_->set_threshold(threshold_);
}


void LFMJammerDetector::init()
{

            detector_cc_->init();
}

void LFMJammerDetector::reset()
{
            detector_cc_->set_active(true);
}


void LFMJammerDetector::set_state(int state)
{
    detector_cc_->set_state(state);
}

void LFMJammerDetector::connect(gr::top_block_sptr top_block)
{
...
void LFMJammerDetector::disconnect(gr::top_block_sptr top_block)
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr LFMJammerDetector::get_left_block()
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr LFMJammerDetector::get_right_block()
{
            return detector_cc_;
}
