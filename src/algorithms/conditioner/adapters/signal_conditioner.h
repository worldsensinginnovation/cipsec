 ...
 */
class SignalConditioner: public SDRBlockInterface
{
public:
    //! Constructor
    SignalConditioner(ConfigurationInterface *configuration,
            std::shared_ptr<SDRBlockInterface> data_type_adapt, std::shared_ptr<SDRBlockInterface> in_filt,
            std::shared_ptr<SDRBlockInterface> res, std::string role, std::string implementation);
    ...
    std::shared_ptr<SDRBlockInterface> data_type_adapter(){ return data_type_adapt_; }
    std::shared_ptr<SDRBlockInterface> input_filter(){ return in_filt_; }
    std::shared_ptr<SDRBlockInterface> resampler(){ return res_; }

private:
    std::shared_ptr<SDRBlockInterface> data_type_adapt_;
    std::shared_ptr<SDRBlockInterface> in_filt_;
    std::shared_ptr<SDRBlockInterface> res_;
    std::string role_;
