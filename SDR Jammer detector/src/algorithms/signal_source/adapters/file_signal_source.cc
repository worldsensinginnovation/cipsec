            ...
            if (filename_.compare(default_filename) == 0)
                {
                    ...
                    << std::endl
                    << "and then generate your own SDR Jammer Detector by doing:"
                    << std::endl
            ...
            else
                {
                    ...
                    << std::endl
                    << "but the specified file is unreachable by SDRJD."
                    << std::endl
...
void FileSignalSource::set_rf_freq(double new_rf_freq_hz)
{
    //nothing to do here
}
void FileSignalSource::set_gain(int new_gain_db){
    //nothing to do here
}
                    ...
                    if (dump_)
                        {
                            ...
                            DLOG(INFO) << "connected throttle_ to file sink";
                        }
                }
            else
                {
                    if (dump_)
                        {
                            ...
                            DLOG(INFO) << "connected file_source_ to file sink";
                        }
                }
        }
    else
            ...
            if (enable_throttle_control_ == true)
                {
                    ...
                    if (dump_)
                        {
                            top_block->disconnect(throttle_, 0, sink_, 0);
                            DLOG(INFO) << "disconnected throttle_ to file sink";
                        }
                }
            else
                {
                    if (dump_)
                        {
                            ...
                            DLOG(INFO) << "disconnected file_source_ to file sink";
                        }
                }
        }
    else
