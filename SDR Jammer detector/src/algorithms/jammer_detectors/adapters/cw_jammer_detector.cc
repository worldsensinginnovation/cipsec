...
CWJammerDetector::CWJammerDetector(
    ...
    role_(role), in_streams_(in_streams), out_streams_(out_streams)
{
    ...
    std::string default_dump_filename = "./data/cw_jammer_detector.dat";
    ...
    detector_cc_ = cw_jammer_make_detector_cc(fs_in_, vector_length_, A_, B_,
                                              max_dwells_, dump_, dump_filename_);
...
void CWJammerDetector::set_threshold(float threshold)
{

    threshold_ = threshold;

    DLOG(INFO) << "CW Jammer detector Threshold = " << threshold_;

    detector_cc_->set_threshold(threshold_);
}


void CWJammerDetector::init()
{

            detector_cc_->init();
}

void CWJammerDetector::reset()
{
            detector_cc_->set_active(true);
}


void CWJammerDetector::set_state(int state)
{
    detector_cc_->set_state(state);
}

void CWJammerDetector::connect(gr::top_block_sptr top_block)
{
...
void CWJammerDetector::disconnect(gr::top_block_sptr top_block)
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr CWJammerDetector::get_left_block()
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr CWJammerDetector::get_right_block()
{
            return detector_cc_;
}
