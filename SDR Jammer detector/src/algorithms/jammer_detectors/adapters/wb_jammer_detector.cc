/*!
 * \file wb_jammer_detector.cc
 * \brief Adapts a Wide Band Noise (WB) Jammer Detector block to a 
...
WBJammerDetector::WBJammerDetector(
    ...
    role_(role), in_streams_(in_streams), out_streams_(out_streams)
{
    ...
    std::string default_dump_filename = "./data/wb_jammer_detector.dat";

    DLOG(INFO) << "role " << role;

    item_type_ = configuration_->property(role + ".item_type", default_item_type);

    fs_in_ = configuration_->property("SDRJD.internal_fs_hz", 2048000);
    ...
    detector_cc_ = wb_jammer_make_detector_cc(fs_in_, vector_length_, noise_estimations_,
                                              A_, B_, max_dwells_, dump_, dump_filename_);
...
void WBJammerDetector::set_threshold(float threshold)
{

    threshold_ = threshold;

    DLOG(INFO) << "WB Jammer detector Threshold = " << threshold_;

    detector_cc_->set_threshold(threshold_);
}


void WBJammerDetector::init()
{

            detector_cc_->init();
}

void WBJammerDetector::reset()
{
            detector_cc_->set_active(true);
}


void WBJammerDetector::set_state(int state)
{
    detector_cc_->set_state(state);
}

void WBJammerDetector::connect(gr::top_block_sptr top_block)
{
...
void WBJammerDetector::disconnect(gr::top_block_sptr top_block)
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr WBJammerDetector::get_left_block()
{
    if (item_type_.compare("gr_complex") == 0)
        {
...
gr::basic_block_sptr WBJammerDetector::get_right_block()
{
            return detector_cc_;
}
